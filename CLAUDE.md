...
- [ ] **Animation Assets** — Not started (M4)
...
### M3 — Renderer
- [~] **Swapchain** — Resize/recreate on `OUT_OF_DATE`/`SUBOPTIMAL` works; double-buffered (`max_frames_in_flight = 2`), not explicitly triple-buffered; present mode now actually configurable (`pickPresentMode` in `swapchain.zig` — FIFO when `vsync=true`, MAILBOX-with-FIFO-fallback when `vsync=false`, driven by `strife.ini`)
- [~] **Pipelines** — `pipeline.zig`/`shadow.zig` build pipelines with `VkPipelineRenderingCreateInfo` (no renderpass), depth/cull/blend configured per pipeline; no on-disk `PipelineCache`
- [x] **Command Buffers** — One pool, `vkResetCommandBuffer` per frame, one-shot upload helper (`upload.zig`), sync2 barriers (`VkImageMemoryBarrier2`/`vkCmdPipelineBarrier2`) throughout including the new shadow pass
- [~] **Meshes** — Push-constant model matrix + `vkCmdDrawIndexed` works; no explicit `RenderCommand`/`RenderQueue` — `RenderSystem.update` queries and draws directly from the ECS each frame instead of building an intermediate list
- [~] **Materials** — Binding 0 = UBO, binding 1 = bindless combined-sampler array (not one-sampler-per-material as originally spec'd, but same descriptor shape); UBO is persistently mapped
...
- [ ] **Debug Draw** — Not started — next up

---

### M4 — Animation
- [ ] **Skeletons** — `Skeleton { joint_count, parent_indices, inv_bind_mats }`; `PoseBuffer { transforms[]JointTransform }`; `compute_skin_matrices` forward pass
- [ ] **Animation Player** — `AnimPlayer { clip, time, speed, loop }`; binary search keyframe bracket; lerp Vec3 / slerp Quat; runs before SkeletonSystem
