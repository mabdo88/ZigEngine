...
pub const RenderSystemState = struct {
    ...
    /// Draws every animated entity's joint hierarchy as yellow lines — joint
    /// i's world position is JointWorldComponent.matrices[i]'s translation
    /// column.
    fn drawSkeletons(registry: *Registry) void {
        var it = registry.Query(.{ components.SkeletonComponent, components.JointWorldComponent });
        while (it.next()) |entity| {
            ...
            for (0..sk.joint_count) |i| {
                ...
                const a: @Vector(3, f32) = .{ world_comp.matrices[@intCast(parent)][3][0], world_comp.matrices[@intCast(parent)][3][1], world_comp.matrices[@intCast(parent)][3][2] };
                const b: @Vector(3, f32) = .{ world_comp.matrices[i][3][0], world_comp.matrices[i][3][1], world_comp.matrices[i][3][2] };
