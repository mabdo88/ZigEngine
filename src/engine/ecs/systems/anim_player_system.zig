...
const FireEventCtx = struct {
    entity: Entity,
    registry: *Registry,

    fn fire(self: *FireEventCtx, e: *const clip_mod.AnimEvent) void {
        self.registry.events.emit(.{ .anim_event = .{ .entity = self.entity, .name = e.name } });
    }
};
...
pub const AnimPlayerSystemState = struct {
    pub fn update(self: *AnimPlayerSystemState, registry: *Registry, dt: f32) anyerror!void {
        ...
        while (it.next()) |entity| {
            ...
            if (c.events.len > 0) {
                var fire_ctx = FireEventCtx{ .entity = entity, .registry = registry };
                clip_mod.forEachFiredEvent(c, prev_time, player.time, &fire_ctx, FireEventCtx.fire);
            }
...
test "AnimPlayer fires anim_event through the registry's EventBus as time crosses it" {
    const allocator = std.testing.allocator;
    var reg = Registry.init(allocator);
    defer reg.deinit();
    ...
    var c = clip_mod.AnimationClip{
        ...
        .duration = 1.0,
        .channels = &.{},
        .events = try allocator.dupe(clip_mod.AnimEvent, &.{.{ .time = 0.5, .name = try allocator.dupe(u8, "footstep") }}),
        .allocator = allocator,
    };
    const clip_id = try reg.clip_cache.register(&c);
    c.deinit();
    ...
    const Listener = struct {
        fired_name: ?[]const u8 = null,
        fn cb(ctx: *anyopaque, payload: @import("../event.zig").EventPayload) void {
            const self: *@This() = @ptrCast(@alignCast(ctx));
            self.fired_name = payload.anim_event.name;
        }
    };
    var listener = Listener{};
    try reg.events.subscribe(.anim_event, &listener, Listener.cb);

    var state = AnimPlayerSystemState{};
    try state.update(&reg, 0.6); // crosses the 0.5 event

    try std.testing.expect(listener.fired_name != null);
    try std.testing.expectEqualStrings("footstep", listener.fired_name.?);
}
